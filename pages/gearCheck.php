...
// This is expansion specific
// These are the higher level enchants for the neck/back/rings
$giftEnchant = array(
158899, 158900, 158901, 158902, 158903, /* neck */
158884, 158885, 158886, 158887, 158889, /* cloak */
158914, 158915, 158916, 158917, 158918 /* ring */
);
	...
	if ($result) {
		...
		for ($i = 0; $i < $numRaiders; $i++) {
			...
			*/
			...
			// Report the blizzard item level stat
			echo '<div style="display:table-cell; float: right; margin-top:20px; padding-right:8px;"> Average Item Level: ' . $character['average_item_level'] . ' (' . $character['average_item_level_equipped'] . ')</div>';
			...
			for ($j = 0; $j < sizeof($gearSlot); $j++) {
				...
				*/
				if($character['item_' . $gearSlot[$j] . '_id'] != 0){
					$itemColor = $itemColorArray[$character['item_' . $gearSlot[$j] . '_quality']];
				}
			    ...
			    $wowheadLink = 'http://www.wowhead.com/item=' . $character['item_' . $gearSlot[$j] . '_id'] . '" rel="';
				...
				if($character['item_' . $gearSlot[$j] . '_id'] == 0){
					...
					echo '<a href="' . $wowheadLink . '"><img src="images/icons/' . $character['item_' . $gearSlot[$j] . '_icon'] . '.jpg" width="20" height="20" '. $iconBorder .'> </a>';
				}
				echo '</td>';
				...
				echo ($character['item_' . $gearSlot[$j] . '_id'] == 0) ? 'N/A' : $character['item_' . $gearSlot[$j] . '_level'];
				echo '</td>';	

				////////////////////////////////////////////////////////////////////////////////////////
				// Name
				echo '<td style="padding:0; color:' . $itemColor . ' width: 28%;">';
				echo ($character['item_' . $gearSlot[$j] . '_id'] == 0) ? 'Empty' : '<a href="' . $wowheadLink . '" style="font-size:0.9em; color:' . $itemColor . '; font-weight:normal; font-size:12px">' . $character['item_' . $gearSlot[$j] . '_name'] . '</a>';
				...
				*/
				...
				if($enchant_id > 0){
					...
					$iconBorder = 'style="border:2px solid green; padding:0px; margin:0px; display: block;"';
					if(in_array($enchant->spell_id, $giftEnchant) == false && 
					    ($gearSlot[$j] == "neck" ||
					    $gearSlot[$j] == "back" ||
					    $gearSlot[$j] == "finger1" ||
					    $gearSlot[$j] == "finger2")){
						$iconBorder = 'style="border:2px solid yellow; padding:0px; margin:0px; display: block;"';
						$suggestion = ($suggestion != "" ? $suggestion . " + " : "") . "Need Better Enchant";
						$severity += 2;
					}
				...
				elseif($character['item_' . $gearSlot[$j] . '_id'] == 0 &&
					   $gearSlot[$j] == "neck" ||
					   $gearSlot[$j] == "back" ||
					   $gearSlot[$j] == "finger1" ||
					   $gearSlot[$j] == "finger2" ||
